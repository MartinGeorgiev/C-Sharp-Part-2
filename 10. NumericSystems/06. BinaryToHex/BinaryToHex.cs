...
class HexToBinary
{
    static void Main()
    {
        ...
        if (arrLenght % 4 != 0) //Checking if number`s lenght is 4, 8 , 12, 16 , 20 , 24, 28, 32 
        {
            real = false; 
        }
        ...
        for (int y = 0; y < arrLenght/4; y++)
        {
            ...
            switch (digitString)
            {
                ...
                default: real = false; break;  //i am checking if number have only 0 and 1 digits.
            }
        ...
        if (real == false)
        {
            Console.WriteLine("There is an error!"); // There is an error if number have not only 0 and 1 digits or number`s lenght is not 4, 8 , 12, 16, 20, 24, 28, 32
            Main();
        }
        else
        {
